...
public class MainForm extends JFrame {
    ...
    private ConvertBridge bridge = new NormalConvertBridge(new Rgb(255, 255, 255));
    ...
    public MainForm() {
        ...
        txtGray.addMouseListener(buildCtbListener("灰度值", Rgb::fromGray, NormalConvertBridge::new));
    }
    ...
    private <M> MouseListener buildCtbListener(String name, Function<String, M> strToModel,
                                               Function<M, ConvertBridge> modelToBridge) {
        ...
        return MouseClickListener.buildClicked(e -> {
            if (e.getButton() == MouseEvent.BUTTON1) {
                ...
                Optional<ConvertBridge> bridge = Optional.of(input).map(strToModel).map(modelToBridge);
                if (!bridge.isPresent()) {
     ...
     * @param bridge 转换桥
     */
    private void updateInfo(ConvertBridge bridge) {
        this.bridge = bridge;
        // 信息面板
        ...
        txtGray.setText(String.valueOf(bridge.getGray()));
     ...
     * @param bridge 转换桥
     */
    public void updateAll(ConvertBridge bridge) {
        updateInfo(bridge);
