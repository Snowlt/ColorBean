...
public class ColorFormula {
    ...
    public static Hsb[] GetFormula(Hsb hsb, FormulaType type, Double angle) {
        double[] formulas = ColorFormula.GetFormula(hsb.h(), type, angle);
        return Arrays.stream(formulas).mapToObj(h -> new Hsb(h, hsb.s(), hsb.b())).toArray(Hsb[]::new);
    }
